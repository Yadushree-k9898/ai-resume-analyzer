...
ACCESS_TOKEN_EXPIRE = timedelta(days=24)


# ✅ Signup Schema (Only Required Fields)
class UserCreate(BaseModel):
    full_name: str
    email: EmailStr
    password: str
...
# ✅ Register Route (Stores only required details)
@router.post("/register/", status_code=status.HTTP_201_CREATED)
def register(user: UserCreate, db: Session = Depends(get_db)):
    existing_user = db.query(User).filter(User.email == user.email).first()
    if existing_user:
    ...
    db_user = User(
        ...
        phone_number=user.phone_number,  # Store phone if provided
        role="user"
    )
    db.add(db_user)
    db.commit()
...
# ✅ Profile Update API (For Adding More Details Later)
class UserUpdate(BaseModel):
    username: str | None = None
    job_title: str | None = None
    industry: str | None = None
